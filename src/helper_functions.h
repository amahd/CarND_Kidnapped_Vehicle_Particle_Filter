...
/*
 * Computes the next x coordinate
 * @param xloc x coordinate
 * @param theta current rotation
 * @param vel  current velocity
 * @param yaw_rate current yaw_rate
 * @param time delta time between measurements
 * @output predicted x coordinate
 */

inline double predx(double xloc, double theta, double vel, double yaw_rate, double time) {

	double x = 0;
	x = xloc + (vel/yaw_rate)*( sin(theta + yaw_rate * time)  - sin(theta) );
	return x;
}


/*
 * Computes the next y coordinate
 * @param yloc y coordinate
 * @param theta current rotation
 * @param vel  current velocity
 * @param yaw_rate current yaw_rate
 * @param time delta time between measurements
 * @output prediced y coordinate
 */

inline double predy(double yloc, double theta, double vel, double yaw_rate, double time) {

	double y = 0;
	y = yloc + (vel/yaw_rate)*( -cos(theta + yaw_rate * time)  + cos(theta) );
	return y;
}
/*
 * Computes the next x coordinate
 ...
 * @param yaw_rate current yaw_rate
 * @param time delta time between measurements
 * @output next angle of the car
 */

inline double predth( double theta,  double yaw_rate, double time) {

	double angle = 0;
	angle = theta + (time * yaw_rate);
	return angle;
}
